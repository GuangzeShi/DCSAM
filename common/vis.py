...
class Visualizer:
    ...
    @classmethod
    def visualize_prediction_batch(cls, spt_img_b, spt_mask_b, qry_img_b, qry_mask_b, pred_mask_b, class_name_b, batch_idx, iou_b=None):
        ...
        # class_name_b 是字符串列表，不需要 to_cpu
        for sample_idx, (spt_img, spt_mask, qry_img, qry_mask, pred_mask, class_name) in \
                enumerate(zip(spt_img_b, spt_mask_b, qry_img_b, qry_mask_b, pred_mask_b, class_name_b)):
            iou = iou_b[sample_idx] if iou_b is not None else None
            cls.visualize_prediction(spt_img, spt_mask, qry_img, qry_mask, pred_mask, class_name, batch_idx, sample_idx, True, iou)
    ...
    @classmethod
    def visualize_prediction(cls, spt_imgs, spt_masks, qry_img, qry_mask, pred_mask, class_name, batch_idx, sample_idx, label, iou=None):
        ...
        spt_masked_pils = [Image.fromarray(cls.apply_mask(spt_img, spt_mask, spt_color))
        ...
        pred_masked_pil = Image.fromarray(cls.apply_mask(qry_img.copy(), pred_mask, pred_color))
        qry_masked_pil = Image.fromarray(cls.apply_mask(qry_img.copy(), qry_mask, qry_color))
        ...
        filename = '%d_%d_class-%s_iou-%.2f.jpg' % (batch_idx, sample_idx, safe_class_name, iou)
        merged_pil.save(os.path.join(cls.vis_path, filename))
